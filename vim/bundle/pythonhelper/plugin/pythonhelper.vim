...
def findTag(bufferNumber, changedTick):
	...
	if (nearestLineNumber > -1):
        ...
        if 'tagInfo' in locals():
            vim.command('let w:PHCurrentTag="%s"' % tagInfo.fullName)
	# }}}
	# }}}
...
function! PythonHelperCurrentTag()
    if (exists("w:PHCurrentTag"))
        return w:PHCurrentTag
    else
        return ""
    endif
endfunction
